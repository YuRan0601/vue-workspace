 ...
 <div>
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
      <div class="container-fluid">
        ...
        <!-- 新增按鈕 -->
        <button type="button" class="btn btn-success" @click="showDialog = true">
      ...
      <v-card>
        ...
        <v-card-text>
          <form @submit.prevent="submitForm">
            <!-- 車型名稱 -->
            <v-text-field label="車型名稱" v-model="formData.carModel" required />
            
            <!-- 品牌 -->
            <v-text-field label="品牌" v-model="formData.brand" required />

            <!-- 排氣量 -->
            <v-text-field label="排氣量" v-model="formData.engineDisplacement" required type="number" />

            <!-- 座位總數 -->
            <v-text-field label="座位數量" v-model="formData.seatingCapacity" required type="number" />

            <!-- 車輛類型 -->
            <v-text-field label="車輛類型" v-model="formData.carType" required />

            <!-- 車輛大小 -->
            <v-text-field label="車輛大小" v-model="formData.carSize" required />

            <!-- 描述 -->
            <v-textarea label="車輛描述" v-model="formData.description" rows="3" />

            <!-- 隱藏創建時間欄位 -->
            <v-text-field v-model="formData.createdAt" style="display: none;" :value="formData.createdAt"/>

            <!-- 隱藏更新時間欄位 -->
            <v-text-field v-model="formData.updatedAt" style="display: none;" :value="formData.updatedAt" />
